...
function initialize(array $config, $config_file)
{
  $temp = array();
  ...
  if (isset($config['url']))
  {
     ...
     if (isset($config['id']))
      $id = $config['id'];
     $file = post($config['url'], array('action' => 'load', 'id' => $id)); 
  }
  else if (isset($config['baseurl']) && ($config['baseurl'] != "test" || isset($config['id'])))
  {
     ...
     if (isset($config['id']))
      $id = $config['id'];
     $file = post($config['baseurl'], array('action' => 'load', 'id' => $id)); 
  }
  else
  {
    // test file
  ...
  if (!$isFile)
    $file = json_decode(json_encode((array)simplexml_load_string($file)), 1);
  else
    $file = json_decode(json_encode((array)simplexml_load_file($file)), 1);

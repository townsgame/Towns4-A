...
if($action=="map" or $action=="tree" or $action=="rock" or $action=="finish"){echo("<br>mapgenerator<br>");
    ...
    //======================================================================================
	function polyrand($seed,$from,$to,$x,$y){
		$from=intval($from);
		$to=intval($to);
		...
		if(!$GLOBALS['ss']['polyseed'][$seed]){
			...
			foreach(array('x','y') as $dim){
				$i==0;while($i<$detail){$i++;
					$GLOBALS['ss']['polyseed'][$seed][$dim][$i][0]=rand(-1000,1000)/10;//amplituda
					$GLOBALS['ss']['polyseed'][$seed][$dim][$i][1]=rand(0,1000)/100;//perioda
					$GLOBALS['ss']['polyseed'][$seed][$dim][$i][2]=rand(0,1000)/100;//posuv	
				}
			}
		}
		//--------
    ...
    if($terrain=="t10" and $action=="tree"){
		...
		//----------
        $res=/*generate($post)*/$res37tree[polyrand('tree',0,36,$xx,$yy)].":".rand(1,360);
        $xx=$x+(rand(-50,50)/100);
        $yy=$y+(rand(-50,50)/100);
    ...
    if($terrain=="t5" and $action=="rock"){
		...
		//----------
        $res=/*generate($post)*/$res37rock[polyrand('rock',0,36,$xx,$yy)].":".rand(1,360);
        $xx=$x+(rand(-50,50)/200);
        $yy=$y+(rand(-50,50)/200);
